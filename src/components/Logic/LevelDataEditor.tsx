...
export const LevelDataEditor: React.FC<LevelDataEditorProps> = ({ levels, onChange, renderLevelContent }) => {
  ...
  // Resize all textareas whenever content or collapsed state changes
  useEffect(() => {
    if (!isCollapsed) {
      textareaRefs.current.forEach((el) => { if (el) autoResize(el); });
    }
  }, [localTexts, isCollapsed]);
              ...
              <div key={i} className={styles.entry}>
                ...
                {renderLevelContent ? (
                  ...
                  <textarea
                    ...
                    value={localTexts[i] ?? ''}
                    onChange={(e) => { handleTextChange(i, e.target.value); autoResize(e.target); }}
                    spellCheck={false}
                  />
                )}
              </div>
